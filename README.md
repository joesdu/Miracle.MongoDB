...
* StartUp.cs ConfigureServices中添加如下内容:
```csharp
services.AddMongoDbContext<DbContext>(Configuration);
// 如某些类中需要将ObjectId在数据库中存为字符串格式可做如下调整,调整后插入数据库中的_id字段将是string类型.
services.AddMongoDbContext<DbContext>(Configuration, c => c.AddConvertObjectIdToStringTypes(typeof(Test)));
```
* Net 6 +
```csharp
using example.api;
